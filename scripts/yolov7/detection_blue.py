...
def main():
    img_counter = 0
    rows, cols = (5,2)
    timestamp_array = [[None] * cols]* rows
    while(img_counter < 2):
        ...
        # Get time
        timestamp_array[0][img_counter-1] = datetime.datetime.now()
        date_time = timestamp_array[0][img_counter-1].strftime("%H_%M_%S")
...
def detect(new_source,new_project, date_time,img_name,waypoint,save_img=True):
    ...
    save_dir = Path(project, exist_ok=exist_ok)
